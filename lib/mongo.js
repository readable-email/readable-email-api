...
var connect = require('then-mongo');
...
function MongoDatabase(db, bucket) {
  this.db = connect(db, ['lists', 'messages', 'topics', 'processed', 'premium-beta-list']);
...
MongoDatabase.prototype.addPremiumBeta = function (email) {
  assert(typeof email === 'string', 'email must be a string');
  assert(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]+$/i.test(email), 'Must be a valid email');
  return this.db['premium-beta-list'].update({_id: email}, {_id: email}, {upsert: true});
};
...
MongoDatabase.prototype.getTopic = function (source, topicSlugg) {
  return this.db.topics.findOne({source: source, slug: topicSlugg});
};
